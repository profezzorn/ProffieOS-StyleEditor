...
varying vec3 vTarget;
  ...
  void main() {
    vA = PD(viewMatrix * modelMatrix * vec4(0.0, -55.0-20.0, 0.0, 1.0));
    vB = PD(viewMatrix * modelMatrix * vec4(0.0, 55.0-20.0, 0.0, 1.0));
...
Range Trace(Ray ray, Plane o) {
  ...
  float dist = tmp / div;
  if (div > 0.0) {
    return Range(NEAR(), dist);
  } else {
    return Range(dist, FAR());
  }
}
...
float blade_dist(vec3 pt1, vec3 dir1_un) {
  ...
  float p = Trace(Ray(vA, vB-vA), Plane(pt1, Q)).end;  // point on blade
  p = clamp(p, 0.0, 1.0);
  vec3 bp = pt2 + dir2 * p;
...
void main() {
  ...
  float flyby_point = get_point(eye, dir);
  float cosA = length(cross(normalize(vB - vA), normalize(dir)));
  dist /= 30.0;
  vec3 haze_color = texture2D(iChannel0, vec2(1.0-flyby_point, sqrt(dist) /2.0 / cosA)).rgb;
//  vec3 haze_color = texture2D(iChannel0, vec2(flyby_point, 1.0)).rgb;
  haze_color /= (dist*dist*dist*dist*500.0+1.0);
//  haze_color *= 1.0 - sqrt(dist);

//  vec2 pt = vec2(0.5, flyby_point);
//  vec3 haze_color = textureLod(iChannel0, pt, dist * 30.0 / cosA).rgb / (dist*dist*dist*dist*500.0+1.0);
//  vec3 haze_color = textureLod(iChannel0, pt, dist * 30.0 / cosA).rgb / (dist*dist*dist*dist*500.0+1.0);

  gl_FragColor = vec4(
        0.0, // dist < 5.0 ? 1.0 : 0.0,
        0.0, // dist < 2.0 ? 1.0 : 0.0,
        1.0-sqrt(dist)/5.0, 1.0);

  gl_FragColor = vec4(haze_color.xyz, 1.0);
}
`;
...
//            console.log("MESH");
	  ...
	  const blade_translation = new THREE.Matrix4().makeTranslation(0.0, -20.0, 0.0).multiply(new THREE.Matrix4().makeRotationX(Math.PI));;
  ...
  if (resizeRendererToDisplaySize(renderer)) {
	...
	for (var haze_depth = 0; haze_depth < max_haze_depth; haze_depth++) {
		...
		for (var D = -64; D <= 64; D++) {
		    ...
		    const stride = p * 3;
		    R += pixels[stride+0] * weight;
		    G += pixels[stride+1] * weight;
		    B += pixels[stride+2] * weight;
		    W += weight;
		}
//		W *= 2;
		R /= W;
		G /= W;
		B /= W;
		haze_data[(i + haze_depth * num_leds)*4+0] = Math.round(R * 255);
